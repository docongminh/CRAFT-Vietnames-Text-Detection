...
"""
# class DataLoader_Kuzu_JPN(data.Dataset):
#     """
#         Dataloader for strong supervised training on Kuzushiji
#     """
#     DEBUG = False
...
#         self.path_labels = config.path_labels
#         self.ids = self.__get_list_id_

#         if DataLoader_Kuzu_JPN.DEBUG:
#             self.ids = self.ids[:1000]

#         total_images = len(self.ids)
#         train_image = int(total_images*config.ratio_split)


#         def __get_list_id_(self):
#             return glob.glob(path_images + '/*')
...
class DataLoader_VN_SYNTH(data.Dataset):
    ...
    def __init__(self, type_):
        ...
        self.type_ = type_
        self.dataset_path = config.DataLoader_VN_SYNTH_dataset_path
        self.raw_dataset = h5py.File(self.dataset_path, 'r')
        self.ids = self.__get_list_id__()
        print("test=======", self.raw_dataset['data'])
        if DataLoader_VN_SYNTH.DEBUG:
            self.ids = self.ids[:1000]
        ...
        if DataLoaderSYNTH.DEBUG:
            ...
            if not os.path.exists('cache.pkl'):
                ...
                with open('cache.pkl', 'wb') as f:
                    ...
                    pickle.dump([mat['imnames'][0][0:1000],
                                mat['charBB'][0][0:1000],
                                mat['txt'][0][0:1000]], f)
